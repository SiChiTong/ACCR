...
FSM(ReturnDock)
{
    ...
    FSM_BGN
    {
        FSM_STATE(Stop)
        {
            ...
            FSM_TRANSITIONS
            {
                FSM_ON_EVENT("/ReturnDock/return", FSM_NEXT(Moving))
            }
        }
        FSM_STATE(Moving)
        {
...
TaskResult moveToDock(std::string, const FSMCallContext& c, EventQueue& e)
{
    ...
    goal_dock.target_pose.pose.position.x = goal_point.x;
    goal_dock.target_pose.pose.position.y = goal_point.y;
    goal_dock.target_pose.pose.orientation.w = goal_point.w;

    mbcDock->sendGoal(goal_dock);
    ROS_INFO("Dock pose sent");

    mbcDock->waitForResult(ros::Duration(TIMEOUT));
    if(mbcDock->getState() == actionlib::SimpleClientGoalState::SUCCEEDED && charging)
    ...
    else
    {
        ...
        e.riseEvent(MALFUNCTION); // transit to top layer state Malfunction
    }
    return TaskResult::SUCCESS();
}
