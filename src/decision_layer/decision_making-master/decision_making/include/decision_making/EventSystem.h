...
#define EQ_SPINNER_DIF_RATE 10
	class Spinner{
		...
		void spin(double rate = EQ_SPINNER_DIF_RATE){
			while(events.check_external_ok() and not events.isTerminated()){
				spinOne();
				boost::this_thread::sleep(boost::posix_time::milliseconds(int(1000/rate))); // this line changed by Liang
			}
	...
	void async_spin(double rate=EQ_SPINNER_DIF_RATE, double start_delay=0.0){
		boost::this_thread::sleep(boost::posix_time::seconds(int(start_delay))); // type casted to int by Liang
